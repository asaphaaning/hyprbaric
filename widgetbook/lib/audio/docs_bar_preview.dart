...
enum DocsDestination {
  ...
  hero(itemId: 115, label: 'Home', target: '/#hero'),
  installer(itemId: 116, label: 'Install', target: '/#install'),
  desktop(itemId: 117, label: 'Desktop', target: '/#desktop');
...
///
/// The landing bar has no Rust hub, so reads, opens, dismissals, and any
/// other non-destination intent stay here rather than printing a skipped
/// RINF delivery.
...
class DocsBarPreview extends StatelessWidget {
  ...
  /// The section reported by the host while scrolling; changes never navigate.
  final DocsDestination? activeSection;

  /// Reports hover and focus introductions to the page host.
  final ValueChanged<ModuleHint?>? onModuleHint;
...
///
/// Reuses the production surface and controls, with five bounded workspaces
/// that navigate to landing-page sections and a title beside search.
...
class _DocsSiteBarState extends ConsumerState<_DocsSiteBar> {
  static const List<DocsDestination> _sections = <DocsDestination>[
    DocsDestination.hero,
    DocsDestination.installer,
    DocsDestination.modules,
    DocsDestination.desktop,
    DocsDestination.configurator,
  ];
  ...
  @override
  void didUpdateWidget(covariant _DocsSiteBar oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.activeSection != widget.activeSection) _syncSection();
  }

  void _syncSection() {
    final int index = _sections.indexOf(
      widget.activeSection ?? DocsDestination.hero,
    );
    if (index >= 0) _activeWorkspace = index + 1;
  }
  ...
  void _setActiveWorkspace(int workspace) {
    final int index = (workspace - 1) % _sections.length;
    setState(() => _activeWorkspace = index + 1);
    widget.onNavigate?.call(_sections[index].resolve(widget.baseUrl));
  }
  ...
  void _onClockCommand(CalendarCommand command) {
    ...
    _month.value = switch (command) {
      CalendarCommand.previousMonth => DateTime(month.year, month.month - 1),
      ...
      child: Focus(
        ...
        child: Padding(
          ...
          child: HyprSurface(
            ...
            // The host paints the translucent fill and blurs the page behind this iframe.
                            ...
                            children: <Widget>[
                              HyprInteractionRegion(
                                ...
                                builder: (context, state) => Padding(
                                  ...
                                  child: Text(
                                    'hyprbaric',
                                    style: HyprTypography.globalMenuTitle
                                        .copyWith(
                                          color: state.active
                                              ? HyprColors.text
                          ...
                          child: Row(
                            ...
                            children: <Widget>[
                              for (final SiteModule module in SiteModule.values)
                                SiteModuleButton(
                                  module: module,
                                  onHint: widget.onModuleHint,
                                  onPressed: () => widget.onNavigate?.call(
                                    DocsDestination.modules.resolve(
                                      widget.baseUrl,
                                    ),
                                  ),
                                ),
                              ...
                              LayerShellDropdown(
                                ...
                                buttonBuilder:
                                    (
                                      ...
                                      return ClockButton(
                                        status: _clockViewFor(_month.value),
                                        isOpen: isOpen,
...
///
/// Opens the search panel with the same typography as the global menu.
/// The shortcut hint remains secondary to the navigation label.
        ...
        child: Column(
          ...
          children: <Widget>[
            HyprTextFieldChrome(
              ...
              focusedColor: const Color(0xff0e0e0e),
              borderRadius: BorderRadius.circular(12),
              borderColor: const Color(0xff292c32),
              focusedBorderColor: HyprAmberLight.edge,
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      cursorColor: HyprAmberLight.edge,
                      decoration: InputDecoration.collapsed(
                        hintText: 'Search the docs…',
                        hintStyle: HyprTypography.globalMenuItem.copyWith(
                          color: HyprColors.textFaint,
                        ),
                      ),
                    ),
