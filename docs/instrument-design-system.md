...
The setup preview filters only its wallpaper image, clipped to the preview bounds. It does not sample the surrounding window with a `BackdropFilter`. Desktop translucency remains compositor-owned. The Linux runner currently selects GPU-accelerated Skia to avoid fatal Impeller GLES framebuffer failures; this preserves the instrument materials rather than removing their effects.
