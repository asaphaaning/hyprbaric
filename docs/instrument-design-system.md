...
The setup preview filters only its wallpaper image, clipped to the preview bounds. It does not sample the surrounding window with a `BackdropFilter`. Desktop translucency remains compositor-owned. The Linux runner uses Impeller. Renderer failures must be addressed in the affected rendering path; Skia is not a fallback policy.
